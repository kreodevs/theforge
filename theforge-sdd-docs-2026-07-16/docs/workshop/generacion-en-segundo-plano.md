...
## Cancelar regeneración MDD

- **API:** `POST /projects/:id/mdd/cancel` — idempotente; deduplica doble clic (~2 s).
- **UI:** banner «Regenerando MDD…» con botón **Detener** (Workshop).
- **Comportamiento:** jobs en cola (`waiting`/`delayed`) se marcan fallidos con mensaje «Cancelado por el usuario». Jobs **activos** (worker con lock BullMQ) usan **cancelación cooperativa**: el grafo consulta `cancelledProjects` entre pasos y aborta sin forzar `job.remove()` sobre locks activos.
- **Polling:** el front llama `signalMddPollAbort()` al cancelar para dejar de hacer poll del job.

### Recuperación tras reinicio del API

Al arrancar, `MddQueueService.recoverOrphanedActiveJobs()` marca como **fallidos** los jobs que quedaron `active` en Redis cuando murió el worker anterior (mensaje: «Recuperado tras reinicio del API (job huérfano)»). Si el banner del Workshop quedó colgado, **recarga el proyecto** o consulta `GET /projects/:id/generation-status` y `GET /projects/:id/mdd-jobs/:jobId`.
