...
Arriba a la derecha: **Descargar todo (ZIP)** agrupa los documentos con contenido en un archivo. Con gobernanza generada, el ZIP puede incluir layout **spec-kit** + `docs/agent-governance`.
...
## 5. Documentos: stamps, diagramas y export

### Fechas Creado / Última regeneración

En pestañas con contenido generado (MDD, Spec, entregables…) verás una barra superior con **Creado** y **Última regeneración**. Las fechas vienen del servidor; si editas manualmente el markdown, comprueba que la regeneración refleje tu último cambio antes de confiar en la cascada.

### Diagramas Excalidraw

Los bloques Mermaid del preview pueden mostrarse como **canvas Excalidraw** (por defecto cuando el tipo de diagrama lo permite) o como **SVG**. Usa el toggle en cada diagrama; en diagramas densos, abre **pantalla completa**.

### Progreso de cascada

Al pulsar **Generar entregables**, la columna de chat muestra el panel **Progreso del flujo**: pasos completados, paso activo y pendientes según la oleada de tu complejidad (LOW / MEDIUM / HIGH). El botón puede mostrar `(N/total)` mientras corre la cola.

### Detener regeneración MDD

Si regeneras el MDD en segundo plano, aparece un banner **Regenerando MDD…** con **Detener**. Detener cancela de forma cooperativa el job en el servidor (`POST …/mdd/cancel`); el grafo para entre pasos. Tras cancelar, recarga el proyecto si el estado quedó inconsistente.

### Export spec-kit (Llevar al repo)

Con semáforo en verde y entregables listos, en la columna derecha puedes abrir **Llevar al repo**: descarga un ZIP compatible con [spec-kit](https://github.com/github/spec-kit) (rutas `specs/`, `.specify/`, gobernanza de agentes). Úsalo para handoff al repositorio de implementación.
...
## 7. Cómo organizar varias líneas de trabajo
...
En el repositorio del producto: `docs/notebooklm/STAGE-SDD.md`, `docs/notebooklm/THEFORGE-INDEX.md`, `docs/notebooklm/mdd-lean-migration.md`, `docs/notebooklm/ENTREGABLES-SDD-VALIDACION.md` (flujo SDD y validación). Ayuda in-app: **Generación en segundo plano**.
