...
|-------|---------|-------------------|------------|
| **Proyecto nuevo (SADD)** | Nombre del proyecto, chat con IA (entrevista) | MDD en sesión → Semáforo → Estimación → Entregables (Blueprint, SPEC, Casos de Uso, Historias, API, Flujos, Infra, Tasks) | Workshop: pestañas Entrevista, MDD, Semáforo, Entregables. Backend: `modules/ai-analysis` (MDD lean + DBGA), `modules/ai-orchestrator`, `modules/engine`, `modules/projects`. |
...
| **ai-analysis** | **MDD lean** (LangGraph: Manager, 4 generadores, **Quality Gate**, `graph_populator`), DBGA (Benchmark), estimación en vivo, cola **`theforge-mdd`** (BullMQ). Prompts en `modules/ai-analysis/prompts/`. |
| **ai** / **ai-orchestrator** | Adapters LLM, chat Workshop, supervisor de agentes, entrevista. Delegación MDD al módulo `ai-analysis`. |
...
- **Backend:** NestJS, Prisma (PostgreSQL), LLM vía instancias BYOK o fallback OpenRouter. **Redis + BullMQ obligatorios** en stack de producción (`REDIS_URL` → colas `theforge-mdd` y entregables).
- **Frontend:** React 18, Vite, Tailwind. Proxy `/api` al backend en dev.
- **Despliegue:** Docker (Dokploy-ready): servicios api, web, db (Postgres), **redis cola**, Falkor SDD. Healthchecks, variables de entorno documentadas. Un solo `docker-compose` en raíz.

---

## 9. Capacidades v1 (2026-07)

Release **v1.0.0** — resumen en [THE-FORGE-V1-RELEASE.md](../THE-FORGE-V1-RELEASE.md):

- **Generación documental v2:** cascada por oleadas, cola MDD/entregables, post-pase de exactitud (greenfield).
- **Visor híbrido Mermaid/Excalidraw** en preview de documentos.
- **Export spec-kit:** wizard «Llevar al repo» / ZIP handoff (`specs/`, `.specify/`, gobernanza).
- **Paso 0 / DBGA:** benchmark & gap analysis antes del Spec.
- **Grupos de proyectos** y **merge de proyectos** (`POST /projects/merge`, `project-groups`).
- **Plugins:** extensión sin acoplar al core — [PLUGINS.md](../PLUGINS.md).

Pipeline MDD lean y tiers C/B/A: [mdd-lean-migration.md](mdd-lean-migration.md).

---

## 10. Fuentes de verdad en el repo
