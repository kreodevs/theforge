...
El MDD debe generarse con **exactamente siete secciones**. Esta estructura es la que usan los agentes del pipeline lean (Clarifier, Software Architect, Security, Integration), el **Quality Gate**, el semáforo Workshop, el parser de markdown y el estimador de costos. Cualquier cambio de títulos o numeración debe reflejarse en prompts, `mdd-markdown-parser.ts`, `estimation.service.ts` y `semaphore.service.ts`.

> **Quality Gate vs Semáforo (2026-07):** El **Quality Gate** (`mdd-quality-gate.node.ts`, grafo LangGraph) valida coherencia del MDD durante generación (`{ ok, blockers, gaps[] }`; el Manager re-enruta generadores). El **Semáforo** (`SemaphoreService`) es heurístico sobre JSON normalizado + complejidad — compuerta UI para «Generar entregables». Son complementarios: QG ≠ semáforo VERDE. La **auditoría manual** (`MddManualAudit`, nodo `mdd-auditor` legacy) es opcional aparte. Ver [mdd-lean-migration.md](mdd-lean-migration.md).
...
- **Agente Quality Gate (pipeline lean):** paso determinista + LLM opcional; si `blockers.length > 0` o gaps no reconocidos (`acknowledgeGaps`), el Manager re-enruta generadores (máx. 2 rondas). **Sin** umbrales 85/90 del antiguo Auditor automático en el grafo.
...
- **Architecting Agentic Systems:** El patrón _Structured output / Planner_ exige salida con secciones concretas; los agentes rellenan cada sección según la matriz de delegación. El **Quality Gate** (grafo MDD) valida coherencia antes de persistir; el **Semáforo** (Workshop) aplica las mismas reglas heurísticas sobre el JSON normalizado — son compuertas distintas (ver nota §0).
...
| **Chain (waterfall)**           | El **MDD** se construye con pipeline lean (Clarifier → Software Architect → Security ∥ Integration → Quality Gate). Los entregables posteriores (Blueprint, API, Infra) se generan uno a uno desde el MDD (y Blueprint cuando aplica).                                                                                                                                              | Correcto. El flujo "Generar Entregables" ejecuta Blueprint → API → Flujos → Infra en secuencia; API e Infra reciben además el Blueprint como entrada.         |
...
| **Reflection / Self-Refine**    | El **Quality Gate** (`mdd-quality-gate.node.ts`) revisa coherencia (`{ ok, blockers, gaps[] }`); si hay gaps no reconocidos, el Manager re-enruta generadores (máx. 2 rondas). **Sin** Architect Critic ni umbrales 85/95 del antiguo Auditor en el grafo. Auto-chequeo en prompts del Arquitecto. Para Blueprint, API, Flujos, Infra no hay loop de refinamiento automático. | Parcial. El MDD tiene ciclo QG + re-enrutado; los demás entregables son one-shot. Ver [mdd-lean-migration.md](mdd-lean-migration.md). |
| **Verifier / HITL**             | **Tres capas:** (1) Quality Gate en generación MDD; (2) Semáforo VERDE como compuerta UI antes de "Generar Entregables"; (3) auditoría manual opcional (`MddManualAudit`, nodo legacy). El usuario puede editar cualquier documento en Workshop.                                                                                                                                    | Correcto. No hay verifier automático post-generación para cada entregable; el humano revisa en la UI. QG ≠ Semáforo.                                           |
...
**Conclusión Architecting:** La implementación sigue Plan-then-Execute (MDD como plan) y Chain (orden Blueprint → API → Flujos → Infra; API e Infra usan Blueprint). En el flujo MDD, Reflection = **Quality Gate** + re-enrutado del Manager (sin Critic/Auditor en grafo). No tenemos Reflection en los entregables individuales (Blueprint, API, Infra) ni Verifier automático post-generación; es una decisión aceptable. Ver [mdd-lean-migration.md](mdd-lean-migration.md) para el pipeline lean.
...
| **Analyze SDD ampliado (P1/P2)**                                                 | `analyzeArtifacts`: UC/HU/UX, Spec↔MDD, HU↔UC, Tasks↔Blueprint/API, puente Phase0→BRD→Spec; post-gen reflection en Flujos/Infra; `verifyDeliverable` incluye `logicFlows`.                                                           |
...
| **Estructura 7 secciones obligatoria**                                           | En el **Quality Gate** (determinista): si faltan secciones canónicas, `blockers` incluye `missingSections` y el Manager re-enruta generadores. El **Semáforo** capa calificación si el JSON normalizado carece de secciones esperadas. |
