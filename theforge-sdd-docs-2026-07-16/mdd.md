...
- `requireBrdTobeGate` — flag histórico en Prisma; gate BRD/To-Be **retirado jul 2026** (ya no bloquea MDD)
...
### 4.2 Pipeline MDD (LangGraph lean)

1. **Manager** recibe mensaje del usuario → decide flujo (completo, secciones puntuales o solo §1)
2. **Clarifier** → Sección 1 (Contexto y Alcance)
3. **Software Architect** → Secciones 2, 3, 4, 5
4. **Security** + **Integration** → Secciones 6 y 7 (en paralelo)
5. **Diagram Injector** → Mermaid ER desde SQL
6. **Quality Gate** → `{ ok, blockers, gaps[] }`; Manager re-enruta generadores si hay gaps (máx. 2 rondas)

**Fuera del grafo:** Architect Critic y Auditor automático eliminados (2026-07). Auditoría manual opcional vía `MddManualAuditService`.
...
### 5.2 BRD/To-Be (legacy, gate retirado jul 2026)

El campo `requireBrdTobeGate` persiste en `Project` pero **`enforceLegacyBrdTobeGate` ya no bloquea** streams MDD ni entregables. BRD y To-Be siguen siendo **opcionales** por etapa (`Stage.brdContent`, `toBeManualContent`). Histórico: proyectos con el flag activo emitían evento `blocked` si faltaban aprobaciones.
