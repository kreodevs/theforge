...
| Hook | Uso |
|------|-----|
| **useDashboardGenerationSummary.ts** | Poll adaptativo `GET /projects/generation-summary?ids=…` (5s si hay jobs, 30s si no; pausa en pestaña oculta). Marca carpetas con job MDD/entregables en curso (`ProjectFolderTile`). Solo activo en el panel de proyectos (no Workshop). |
