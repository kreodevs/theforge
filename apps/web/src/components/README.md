...
| **MarkdownMermaid** | Bloques Mermaid en markdown (MDD, tutorial, ayuda). **Fuente:** siempre DSL Mermaid en el documento. **Preview:** `resolveMermaidBlockForRender` (SSOT) aplica reparación determinista antes de render; Excalidraw por defecto si el tipo es convertible; SVG Mermaid en fallback. `assessMermaidFixStrategy` solo pide LLM si la reparación local no valida. Reparar/Regenerar (izq.), toggle Excalidraw/SVG, pantalla completa (der.). |
