...
| **MarkdownMermaid** | Bloques Mermaid en markdown (MDD, tutorial, ayuda). **Fuente:** siempre DSL Mermaid en el documento. **Preview híbrida:** Excalidraw por defecto si el tipo es convertible (`flowchart`, `erDiagram`, `sequenceDiagram`, `classDiagram`); SVG Mermaid en fallback (tipos no soportados, error de conversión o toggle manual). Detección unificada en `mermaid-diagram-type.util.ts`. Al editar el markdown fuente se reintenta Excalidraw automáticamente. Reparar/Regenerar (izq.), toggle Excalidraw/SVG, pantalla completa (der.). |
| **ExcalidrawDiagramBlock** | Convierte Mermaid → canvas Excalidraw (lazy). Flowcharts editables; ER/secuencia/clase como imagen embebida. Rebuild manual y export PNG. |
