...
| **DocumentClarificationSection** | Banner + respuestas para `[NEEDS CLARIFICATION]` en entregables del Workshop (`clarify-document` / `resolve-clarifications`). Integrado en `StandardDocPanel` y pestañas MDD, Fase 0, BRD, Spec. |
| **ClarifyDocumentPanel** | Diálogo «Aclarar» (equivalente `/speckit.clarify`) por tipo de documento. |
