...
| **MddViewer** | Preview markdown (Fase 0, MDD, BRD, Blueprint): `repairDirectoryTreeBlocks` + detección `((Root))`/`— apps/` envuelve árboles en ` ```text `; párrafos colapsados → `<pre>` monoespaciado. Diagramas Mermaid vía `MermaidDiagramBlock` (híbrido Excalidraw/SVG). |
...
| **MarkdownMermaid** | Bloques Mermaid en markdown (MDD, tutorial, ayuda). **Fuente:** siempre DSL Mermaid en el documento. **Preview:** `resolveMermaidBlockForRender` (SSOT) aplica reparación determinista antes de render; Excalidraw por defecto si el tipo es convertible (mismo `prepareContent` que el SVG); SVG Mermaid en fallback (`onFallbackToSvg` estable). `assessMermaidFixStrategy` solo pide LLM si la reparación local no valida. Reparar/Regenerar (izq.), toggle Excalidraw/SVG, pantalla completa (der.). |
| **ExcalidrawDiagramBlock** | Convierte Mermaid → canvas Excalidraw (lazy + CSS). Contenedor con altura explícita (Excalidraw = `height:100%`). Pasa `elements` **y** `files` (ER/secuencia/clase = imagen). Remount con `sceneRev` tras rebuild (`initialData` solo aplica al montar). Flowcharts editables; export PNG incluye files. |
