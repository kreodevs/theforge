...
| **MarkdownMermaid** | Bloques Mermaid en markdown (MDD, tutorial, ayuda). **Fuente:** siempre DSL Mermaid en el documento. **Preview:** Excalidraw por defecto si el tipo es convertible; SVG en fallback. Toolbar **en flujo** (Reparar · SVG/Excalidraw · Pantalla completa) sin solaparse con el canvas. |
| **ExcalidrawDiagramBlock** | Mermaid → Excalidraw (lazy + CSS, altura explícita, `files`). Controles **abajo a la derecha**: zoom −/+/ajustar, rebuild, editar, export PNG. Chrome nativo de Excalidraw oculto (`zenMode` + CSS) para no tapar la toolbar del Workshop. |
