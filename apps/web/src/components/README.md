...
| **MarkdownMermaid** | Bloques Mermaid en markdown (MDD, tutorial, ayuda). Botón **Pantalla completa** + modal con **fit-to-view** al abrir, **zoom** (rueda / +/-) y **pan** (arrastrar). Doble clic o ↺ restablece encuadre. |
...
| **DesignRefSelector** | Biblioteca visual en pestaña **Design System**: catálogo `GET /api/design-refs`, badge `DESIGN.md` si hay import; al cambiar referencia con Design System ya generado se dispara regeneración (`handleDesignRefChange` → `generateUxGuideSequential`). |
