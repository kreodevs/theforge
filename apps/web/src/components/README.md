...
| **ExcalidrawDiagramBlock** | Mermaid → Excalidraw. Chrome nativo oculto. Canvas alineado al papel del preview (`viewBackgroundColor` = mix muted/card; tema light/dark según `<html>`). Al cargar y al redimensionar el panel hace **fit al contenido** (`scrollToContent` + `fitToContent`); botón ↺ en toolbar para re-ajustar. Gestos nativos; −/+ al centro; toolbars opacas. |
