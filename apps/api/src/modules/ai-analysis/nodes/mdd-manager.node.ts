  ...
  return async (state: MDDStateType): Promise<Partial<MDDStateType> | Command> => {
    ...
    const context = [
      ...
      state.lastStepFailed
        ...
        : "",
      userMessage
        ? `\n**Mensaje actual:**\n${userMessage}\n\n**Instrucción:** Clasifica en una de las intenciones del prompt.\n\n**REGLAS PARA PREGUNTAS:** Si el usuario pregunta "para qué", "por qué", "qué es", "cómo funciona", "dónde se usa" o cualquier pregunta factual sobre el contenido del MDD (tecnologías, tablas, endpoints, infraestructura), responde con "reply" — es una consulta informativa, NO un cambio. No trates preguntas como solicitudes de modificación.\n\n**REGLAS PARA CAMBIOS:** Si el mensaje describe una **necesidad** (pantalla, tabla, endpoint, flujo, MFA, etc.) que afecta a uno o más agentes, devuelve \`target: "sections"\` y \`sections: [lista de agentes]\` (software_architect, security, integration). Si es solo aclaración → "reply"; si es confirmar o información amplia → "delegate" (pipeline completo). Si el usuario **acaba de responder** a preguntas que hiciste (mensaje con contenido concreto, no solo "sí"/"no"), **delega** para incorporar su respuesta al documento; no hagas otra pregunta de seguimiento. Si el mensaje es corto y usa "lo", "eso", "elimínenlo" y en Respuestas del usuario o en el Borrador se mencionó algo concreto (ej. Kubernetes, una tecnología), **delega** con esa directiva; no respondas pidiendo "qué especificar" o "qué eliminar". Ante la duda, "reply".`
        : "",
    ]
      .filter(Boolean)
      .join("\n");
