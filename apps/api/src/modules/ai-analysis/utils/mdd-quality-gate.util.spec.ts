...
- Auditoría registra cada intento fallido.`;
  ...
  it("mddQualityGateHasBlockers refleja blockers del gate", () => {
    assert.equal(mddQualityGateHasBlockers(VALID_MDD), false);
...
describe("runDeterministicMddQualityGate", () => {
  it("expone gaps estructurados cuando faltan secciones", () => {
    const draft = "## 1. Contexto\nAlgo.";
    const det = runDeterministicMddQualityGate(draft);
    assert.ok(det.gaps.length > 0);
    assert.ok(det.blockers.length > 0);
    assert.equal(shouldSkipLlmQualityGate(det), false);
  });

  it("shouldSkipLlmQualityGate cuando no hay blockers ni gaps", () => {
