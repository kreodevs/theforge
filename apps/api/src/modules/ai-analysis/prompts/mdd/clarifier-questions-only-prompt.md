...
- **Stack no reflejado en Infra (feedback tipo "Arquitectura↔Infra" o "Dockerfile Node"):** El stack **ya está definido en §2** (Arquitectura). **No** preguntes "¿cuál es tu preferencia para el stack?" ni "¿qué stack en infra?". **Propón** en función de §2: ej. "En §2 el backend es NestJS/Node. Proponemos añadir en §7 un Dockerfile con Node 20 y health check. ¿Lo validas o prefieres otra imagen base?" Pide validación, no información que ya está en el documento.
