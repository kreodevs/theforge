...
**Fallback local:** si el LLM no devuelve JSON válido al responder, `phase0-assisted-fallback.util.ts` incorpora la respuesta en borrador/markdown, re-sincroniza el borrador desde el markdown (`refreshBorradorFromWorkingMarkdown`) y avanza el plan de gaps. Si el usuario **pide ayuda para generar** UAT/riesgos (`phase0-assisted-help.util.ts`), se infiere desde el documento o se responde que falta información **sin consumir el turno**. Al **iniciar** modo asistido, la heurística DBGA se fusiona con la extracción LLM para no perder §1 **Problema:** ya escrito.
