...
- **graph/mdd-graph.ts** – Grafo: Manager → … **Perf (pasada completa):** con `MDD_TAIL_PARALLEL` (default on), tras `format_after_architect` el nodo **`tail_parallel`** ejecuta §5 ∥ §6 ∥ §7 (`createMddTailParallelNode`). Sin flag (`MDD_TAIL_PARALLEL=0`), **`security_integration`** (Promise.all §6+§7). Nodos individuales se conservan para `sectionsToRun` y delivery gate.
