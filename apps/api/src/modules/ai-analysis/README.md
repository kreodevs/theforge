...
- **state/** – Estado compartido LangGraph (tipado estricto)
  ...
  - `index.ts` – Re-export
- **llm/** – `create-dbga-llm.ts` – Runtime BYOK/tenant del usuario (`resolveRuntime`). `createMddAuditorLLM` usa `resolveAuditorRuntime` (`auditorChatModel` de la instancia activa del proveedor, si no el mismo `chatModel`).
