...
- `POST /projects/:projectId/legacy/generate-deliverables` — Despacho dinámico según `Project.complexity`: solo los pasos en `DELIVERABLES_BY_COMPLEXITY`, con contexto TheForge inyectado. **Fuente:** `mddContent` (MDD de cambio) o, si está vacío, `legacyFlowState.codebaseDoc` (MDD Inicial → ingeniería inversa). Si hay `complexityPending` sin confirmar, 400. **Respuesta:** `{ ok, lastDeliverablesDebug }` — traza de pasos (duración, `outChars` por entregable, errores). La misma traza se persiste en **`legacyFlowState.lastDeliverablesDebug`** (última ejecución). Logs Nest por paso: `LEGACY_DELIVERABLES_DEBUG=1`. Resumen INFO al terminar: `[LegacyDeliverables] cascade_ok …`. **Throttle (429 TPM/RPM):** entre cada paso LLM consecutivo se espera **`LEGACY_DELIVERABLES_INTER_STEP_DELAY_MS`** (default **3500**; `0` desactiva). Si **`mddChars`** supera **`LEGACY_DELIVERABLES_LARGE_MDD_THRESHOLD_CHARS`** (default **100000**), antes del primer entregable LLM se aplica **`LEGACY_DELIVERABLES_LARGE_MDD_COOLDOWN_MS`** (default **12000**). **429 upstream (p. ej. TPM Moonshot / Gemini):** cuerpo `{ statusCode: 429, code: "UPSTREAM_LLM_RATE_LIMIT", message, retryAfterSeconds, lastDeliverablesDebug }` (traza con `upstreamRateLimited` / `retryAfterSeconds`); subir cuota, ajustar env de throttle o reintentar tras la espera.
