...
1. **No inventes; apégate al MDD y al conocimiento TheForge.** Usa solo la información que te proporcionen. El contexto TheForge puede incluir: bloque **evidencia del índice** (búsqueda semántica + símbolos por archivo + extractos de archivos prioritarios), impacto y contratos (`validate_before_edit`), definiciones exactas (`get_definitions`), funciones por archivo (`get_functions_in_file`), hits de búsqueda semántica (`semantic_search`). Prioriza hechos anclados a rutas o símbolos explícitos; la síntesis NL debe coincidir con esa evidencia. No inventes props, firmas, dependencias ni estructura.
