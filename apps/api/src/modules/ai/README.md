...
- **`config/llm-config.ts`** — Runtime **OpenRouter** (chat + embeddings vía la misma API), `getLlmProvidersSnapshot()`. `LLM_EMBEDDINGS_PROVIDER=none` apaga embeddings.
- **`adapters/openrouter.adapter.ts`** — `OpenRouterAdapter`: `chat.completions` y `embeddings` con SDK `openai` (base `OPENROUTER_BASE_URL`).
