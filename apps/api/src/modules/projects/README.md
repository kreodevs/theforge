...
- **BRD/To-Be greenfield:** `POST /projects/:id/suggest-brd-tobe-from-dbga` (body opcional `{ stageId?: string }`) — borradores desde `Project.dbgaContent` (mín. ~300 caracteres); rechaza `projectType === LEGACY` (usar `POST …/legacy/suggest-brd-tobe-from-codebase-doc`). Mismo contrato de delimitadores `<<<BRD>>>` / `<<<TOBE>>>` que el endpoint legacy; anula aprobaciones en la etapa destino.
