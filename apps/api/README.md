...
- **IA:** un solo proveedor, **OpenRouter** (`https://openrouter.ai/api/v1` compatible OpenAI). Config: `modules/ai/config/llm-config.ts`; `OpenRouterAdapter` + `createDbgaLLM` comparten `resolvePrimaryChatRuntime`. Clave: `OPENROUTER_API_KEY` o alias `AI_API_KEY` / `OPENAI_API_KEY`. Chat: default `nousresearch/hermes-3-llama-3.1-405b` (`OPENROUTER_CHAT_MODEL`). Embeddings: mismo endpoint, default `openai/text-embedding-3-small` (`OPENROUTER_EMBEDDING_MODEL`); `LLM_EMBEDDINGS_PROVIDER=none` desactiva.
