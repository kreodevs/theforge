...
- **IA:** proveedor activo `LLM_PRIMARY_PROVIDER` o `AI_PROVIDER` (`openai` \| `google` \| `gemini` \| `kimi` \| `moonshot`). Config central `modules/ai/config/llm-config.ts`: mismo runtime para adapters y `createDbgaLLM`. Kimi usa `OPENAI_API_KEY` + defaults Moonshot (`OPENAI_BASE_URL` / `OPENAI_CHAT_MODEL` opcionales). Embeddings: `LLM_EMBEDDINGS_PROVIDER` opcional; con chat Kimi y clave Google, embeddings vía Gemini.

Env: `DATABASE_URL`, claves existentes (`OPENAI_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY` / `GEMINI_API_KEY`). Ver `.env.example`. **Auth (Fase 1):** Passport **`JwtStrategy`** + `JwtAuthGuard` global; `UserContextInterceptor` + `AsyncLocalStorage` propagan `userId` del JWT a `ProjectsService` / `SessionsService` (filtrado por propiedad). `JWT_SECRET` obligatorio en producción. **OTP:** `EMAIL_OTP` (recomendado en Docker/Dokploy) o `AUTH_ALLOWED_OTP_EMAIL` — único correo que recibe el código; en producción uno de los dos es obligatorio al arranque. SMTP como en `.env.example`; tras verify, `User` en BD y **`sub` en JWT = `User.id`**.
