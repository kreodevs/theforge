...
- **`theforge-doc-stamp.ts`:** peel/reattach del stamp `<!-- theforge-doc:created|updated -->` + blockquote 📅; `peelDocumentBodyForPersist`, `extractCanonicalMddBody` y `repairInlineHorizontalRuleSectionBreaks` limpian stamps pegados/corruptos (incl. §1–§2 inline en la zona de fechas) antes de persistir o mostrar en Workshop. `extractCanonicalMddBody` recorta residuo de stamp hasta `# MDD`, gobernanza SSOT, §1 o §2 — nunca salta a §3+ (evita perder §1/§2 en regen completa). El peel del blockquote 📅 solo consume la línea de fechas + `---` del stamp (no el `---` de cierre de la sección SSOT de patrones).
