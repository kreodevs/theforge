...
- **`theforge-doc-stamp.ts`:** peel/reattach del stamp `<!-- theforge-doc:created|updated -->` + blockquote 📅; `peelDocumentBodyForPersist`, `extractCanonicalMddBody` y `repairInlineHorizontalRuleSectionBreaks` limpian stamps pegados/corruptos (incl. §1–§2 inline en la zona de fechas) antes de persistir o mostrar en Workshop. El peel del blockquote 📅 solo consume la línea de fechas + `---` del stamp (no el `---` de cierre de la sección SSOT de patrones).
