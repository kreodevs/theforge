...
- **SDD — precisión en generación (cascada, checklist y validadores cross-artifact):** la cascada de entregables greenfield deja el paralelismo total por **oleadas secuenciales** (`DELIVERABLE_WAVES_BY_COMPLEXITY` en `@theforge/shared-types`): W1 spec+architecture → W2 UC/HU/API/flujos/UX/blueprint → W2b sync pantallas → W3 tasks/infra/gobernanza → W4 post-pase con retry dirigido. Nuevo checklist greenfield (`sdd-coverage-checklist.util.ts`) extrae del MDD §2–§5, phase0/research, blueprint y open gaps `[OPEN-GAP]` e inyecta bloque de cobertura en generadores (`generateArchitecture`, `generateTasks`, `generateUseCases`, `generateLogicFlows`, `generateApiContracts`, `generateBlueprint`); `phase0SummaryContent` / `phase0GapsJson` propagados vía `LegacyGenerateOptions`. Validadores deterministas (`sdd-precision-checks.util.ts`): architecture vs MDD §2, migraciones por columna SQL, fases blueprint en tasks, scheduler canónico, research→tasks, RabbitMQ, schemas Zod en UC, pantallas vs logic-flows; integrados en `analyzeArtifacts`, `collectConformanceGaps` y post-pase W4 (`precisionGapsForPostPassRetry`). Semáforo HIGH baja a **AMARILLO** (score ≤ 82) si quedan gaps cross-artifact; `PROGRESO.md` incluye sección **Gaps SDD pendientes**. Prompts endurecidos (architecture, tasks, logic-flows, use-cases, phase0-deep-research). Tests de regresión con fixtures genéricos en `__fixtures__/ia-trading-gaps/`.
...
- **SDD cascada — W4 más rápido:** eliminados reintentos inline duplicados en `generateArchitecture` y `generateTasks` (un solo pase por oleada); la corrección por gaps de precisión queda centralizada en post-pase W4. W4 ejecuta architecture, logic-flows y api-contracts **en paralelo** y **tasks al final** (depende del upstream actualizado). eliminado hardcode Kreo en prompts, enrich MDD/Blueprint, markdown de pantallas y reglas SDD; columna dinámica **Componentes ({libraryName})** o **Componentes UI**; anexo catálogo sin duplicar tokens.
