...
### 3.3 Orquestación multiagente (LangGraph)

| Componente | Rol |
|---|---|
| **Manager** | Orquesta flujo: delegates a Clarifier, Arquitecto, Security, Integration, Auditor |
| **Clarifier** | Sección 1 (Contexto y Alcance) + preguntas de refinamiento |
| **Software Architect** | Secciones 2–5 (Stack, Modelo, API, Lógica) |
| **Security Architect** | Sección 6 (Seguridad) |
| **Integration Engineer** | Sección 7 (Infraestructura) |
| **Architect Critic** | Verifica §3/§4 contra directiva; loop de refinamiento |
| **Auditor** | Evalúa las 7 secciones; feedback y score |
...
- **BRD/To-Be gate:** `enforceLegacyBrdTobeGate` — si `requireBrdTobeGate`, exige BRD y To-Be aprobados antes de generar MDD técnico
...
3. **IA siempre vía OpenRouter** — no importes SDKs de LLM fuera de `adapters/`.
4. **FalkorDB y Ariadne MCP son cosas distintas** — no mezcles URLs ni contractos.
5. **BRD/To-Be gate** — si `requireBrdTobeGate=true`, no generes contenido técnico sin aprobación.
