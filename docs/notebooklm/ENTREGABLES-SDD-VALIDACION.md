...
| **Integration Spec**                | **Integration Spec** (integrationSpecContent) | Contrato de frontera entre sistemas (SSO, webhooks, terceros). Se genera desde MDD (+ Blueprint/API opcional). Distinto de Infra (DevOps) y de Flujos (lógica interna). HIGH/MEDIUM en matriz; omitido en LOW.                              |
...
- **Implementación actual:** En "Generar Entregables" la cascada paralela cubre la mayoría de pasos; **`integration_spec` corre secuencialmente tras `api_contracts`** (depende de contratos API si existen). Orden en matriz HIGH: … → API → Integration Spec → Flujos → … → Infra. API e Infra reciben el MDD de la **etapa activa** (`Stage`) y `blueprintContent` del proyecto.
...
| **Integration Spec** | Contratos de frontera / integración      | Un LLM desde MDD (+ Blueprint/API opc.); markdown con mapa de sistemas, §3–§7 ISD   | Sí       |
...
| **Conformance Integration Spec vs §1/§4.B/§7**                                   | `checkIntegrationSpecVsMdd` (6 heurísticas + LLM opcional); GET conformance incluye `integrationSpec`; pestaña Workshop «Integración»; caso N/A sin §4.B.                                                                                  |
| **Regenerar con gaps**                                                           | Parámetro `gapsFeedback` en generateBlueprint, generateApiContracts, generateIntegrationSpec, generateLogicFlows, generateInfra; prompts reciben "Los siguientes puntos deben corregirse..."; botones "Regenerar X con gaps" en panel Conformance cuando hay gaps. |
