...
**Uso desde la API:** La conectividad con TheForge desde el backend requiere un **token M2M**. Se configura con la variable de entorno `THEFORGE_M2M_TOKEN`; el cliente debe enviarlo en cada petición al MCP (p. ej. header `Authorization: Bearer <token>`). No commitear el token; solo en `.env` o secrets del despliegue. Ver `PLAN-IMPLEMENTACION-THEFORGE-WEB.md`.
